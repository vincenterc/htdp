...
; LTracks -> N
; produces the total amount of play time given a list of tracks lt
(check-expect (total-time ltracks1) 0)
(check-expect (total-time ltracks2) 424776)
(check-expect (total-time ltracks3) 642872)
(define (total-time lt)
  (cond [(empty? lt) 0]
        [else (+ (total-time (rest lt))
                 (track-time (first lt)))]))

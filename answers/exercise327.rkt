...
; (define (create-bst-from-list llns)
;   (cond [(empty? llns) NONE]
;         [else (create-bst
;                (create-bst-from-list (rest llns))
;                (first (first llns))
;                (second (first llns)))]))
(define (create-bst-from-list llns)
  (foldr (lambda (lns bst)
           (create-bst bst (first lns) (second lns)))
         NONE
         llns))

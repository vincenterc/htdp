...
(check-expect (find-path 'A 'G sample-graph)
              '(A B E F G))
(check-expect (find-path 'B 'D cyclic-graph)
              '(B E C D))
(define (find-path origination destination G)
  (local (; Node [List-Node] -> [Maybe Path]
          (define (find-path-aux origin seen)
            (cond
              [(symbol=? origin destination) (list destination)]
              [(member? origin seen) #false]
              [else (local ((define next (neighbors origin G))
                            (define candidate
                              (find-path/list next (cons origin seen))))
                      (cond
          ...
          (define (find-path/list lo-Os seen)
            (cond
              ...
              [else (local ((define candidate
                              (find-path-aux (first lo-Os) seen)))
                      (cond
                        [(boolean? candidate)
                         (find-path/list (rest lo-Os) seen)]
                        [else candidate]))])))
    (find-path-aux origination '())))

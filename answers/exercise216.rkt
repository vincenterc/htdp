...
; Worm -> Boolean
; stops if the worm has run into the walls
(check-expect (end? INITIAL-WORM) #false)
(check-expect
 (end? (make-worm (make-posn 0 20) LEFT))
 #true)
(check-expect
 (end? (make-worm (make-posn WIDTH 20) RIGHT))
 #true)
(check-expect
 (end? (make-worm (make-posn 30 0) UP))
 #true)
(check-expect
 (end? (make-worm (make-posn 30 WIDTH) DOWN))
 #true)
(define (end? w)

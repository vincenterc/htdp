...
(check-expect (render 150) (place-image CAR (- 150 HALF-CAR-WIDTH) Y-CAR BACKGROUND))
(check-expect (render 200) (place-image CAR (- 200 HALF-CAR-WIDTH) Y-CAR BACKGROUND))
(define (render ws)
  (place-image CAR (WS-TO-X-CAR ws) Y-CAR BACKGROUND))

(define (WS-TO-X-CAR ws)
  (- ws HALF-CAR-WIDTH))

; WorldState -> Boolean
; end when ws is bigger than X-CAR-STOP
(check-expect (end? (/ BACKGROUND-WIDTH 2)) #false)
(check-expect (end? (+ BACKGROUND-WIDTH CAR-WIDTH 1)) #true)
(define (end? ws)
  (> ws X-CAR-STOP))
